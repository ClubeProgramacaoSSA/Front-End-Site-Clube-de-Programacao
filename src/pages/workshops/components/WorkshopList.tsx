...
export function WorkshopList({ workshops }: WorkshopListProps) {
  const [selectedWorkshop, setSelectedWorkshop] = useState<ContentProps>(workshops[0]);
  return (
    <>
      <Listbox
        as="section"
                    ...
                    return (
                      <Listbox.Option value={workshop} key={index}>
                        {({ active, selected }) => {
                          ...
                          let lastItem = index == workshops.length - 1;
                          return (
                            <li
                              className={`
                                ${
                            ...
                            >
                              ...
                              <div className="flex gap-3">
                                ...
                                />
                                <span className="text-2xl">{workshop.workshopName}</span>
                              </div>
                            </li>
                          );
                        }}
